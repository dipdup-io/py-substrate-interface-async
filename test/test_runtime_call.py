...
class RuntimeCallTestCase(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_core_version(self):
        result = await self.substrate.runtime_call("Core", "version")
    ...
    async def test_transaction_payment(self):
        ...
        extrinsic = await self.substrate.create_signed_extrinsic(call=call, keypair=self.keypair, tip=1)
        extrinsic_len = await self.substrate.create_scale_object('u32')
        extrinsic_len.encode(len(extrinsic.data))

        result = await self.substrate.runtime_call("TransactionPaymentApi", "query_fee_details", [extrinsic, extrinsic_len])

        self.assertGreater(result.value['inclusion_fee']['base_fee'], 0)
        self.assertEqual(0, result.value['tip'])

    async def test_metadata_call_info(self):

        runtime_call = await self.substrate.get_metadata_runtime_call_function("TransactionPaymentApi", "query_fee_details")
        param_info = runtime_call.get_param_info()
        self.assertEqual('Extrinsic', param_info[0])
        self.assertEqual('u32', param_info[1])

        runtime_call = await self.substrate.get_metadata_runtime_call_function("Core", "initialise_block")
        param_info = runtime_call.get_param_info()
        self.assertEqual('u32', param_info[0]['number'])
        self.assertEqual('h256', param_info[0]['parent_hash'])

    async def test_check_all_runtime_call_types(self):
        runtime_calls = await self.substrate.get_metadata_runtime_call_functions()
        for runtime_call in runtime_calls:
            param_info = runtime_call.get_param_info()
            self.assertEqual(type(param_info), list)
            result_obj = await self.substrate.create_scale_object(runtime_call.value['type'])
